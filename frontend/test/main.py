...
msg_card_tail_jp = ']/div[1]/div[@class="message-card-text"]/div'
native_message_list = []
jp_message_list = []
DOM_list = []
...
for message in api_json["messages"]:
    ...
    driver.execute_script("arguments[0].scrollIntoView();", driver.find_element_by_xpath(jp_msg_xpath))
    DOM_list.append({"country": str(driver.find_elements_by_class_name("undefined")[api_json["messages"].index(message)].get_attribute("alt")),
